  ...
  return (
    <header className='shadow-md fixed z-50 top-0 border-b border-blue-400 font-sans w-screen flex justify-center bg-blue-400'>
        <nav className='px-2 py-1 flex flex-row items-center justify-center w-11/12'>
            ...
            {/* LOGIN AND ACCOUNT */}
            <div className=' text-slate-900 py-2 ml-auto text-lg flex justify-center items-center'>
                    ...
                    {(loginHovered)?
                        <div className='shadow-lg shadow-slate-300 bg-white rounded-sm h-max absolute top-14 w-1/5 -translate-x-4 flex flex-col justify-center'>
                            <div className='flex justify-between items-center w-full border-b-2 border-slate-300 p-2'>
                                ...
                                <Link to={isAuthenticated? "/logout" : "/customer/register"} 
                            ...
                            {(!role || (role && (role === "CUSTOMER" || role === "SELLER")))
                ...
                {/* CORE MODULE LINK */}
                <Link to={
                    !isAuthenticated? "/seller/register" : 
                    (role === "SELLER")?"/orders":
                    (role === "CUSTOMER")? "/cart" :
                    (role === "ADMIN")? "" : ""}
                ...
                >
                    ...
                    <p className='px-1'>
                        {!isAuthenticated? "Become a Seller" :
                        (role === "SELLER")?"Orders":
                        (role === "CUSTOMER")? "Cart" :
                        (role === "ADMIN")? "" : ""}
                    </p>
